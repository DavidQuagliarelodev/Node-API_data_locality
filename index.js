...
const timezone = require("./lib/timezone/api/timezone");
...
app.get(
  "/login/:userLogin/:tokenLogin/:pais/:moeda/:area/:location",
  async (req, res) => {
    const userLogin = req.params.userLogin;
    const tokenLogin = req.params.tokenLogin;
    if (
      ...
      const resultTime = await timezone
        .timeGenerate(area, location)
        .then((e) => {
          return e;
        });
      const integration = {
